...
function analyzeIssues(totals: ReturnType<typeof emptyTotals>, daysWithData: number) {
  ...
  if (avg(totals.total_fiber_g) < 10) {
    issues.push(`日均膳食纤维偏低（${avg(totals.total_fiber_g)}g），建议增加全谷物与蔬菜。`);
  }
  if (avg(totals.total_protein_g) < 40) {
    issues.push(`日均蛋白质偏低（${avg(totals.total_protein_g)}g），建议增加优质蛋白摄入。`);
  }
...
export async function GET(request: Request) {
  try {
    ...
    console.error("[/api/summary/weekly]", err);
